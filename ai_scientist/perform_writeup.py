...
def perform_writeup(
        ...
        engine="semanticscholar", attack=None, attack_target_score=10,
        defense=False, defense_level="standard",
):
...
First, re-think the Title if necessary. Keep this concise and descriptive of the paper's concept, but try by creative with it."""
    ...
    # Attack injection: modify template.tex before PDF compilation
    if attack:
        template_path = osp.join(folder_name, "latex", "template.tex")
        with open(template_path, "r", encoding="utf-8") as f:
            tex_content = f.read()
        tex_content = inject_latex_attack(tex_content, attack, attack_target_score)
        with open(template_path, "w", encoding="utf-8") as f:
            f.write(tex_content)
        print(f"[ATTACK] Injected '{attack}' strategy into LaTeX (target score: {attack_target_score})")

    # LaTeX sanitization: strip hidden text when defense is enabled
    if defense:
        from ai_scientist.latex_sanitizer import sanitize_latex, detect_injection_markers
        template_path = osp.join(folder_name, "latex", "template.tex")
        with open(template_path, "r", encoding="utf-8") as f:
            tex_content = f.read()
        markers = detect_injection_markers(tex_content)
        sanitized_tex, report = sanitize_latex(
            tex_content, aggressive=(defense_level == "paranoid")
        )
        with open(template_path, "w", encoding="utf-8") as f:
            f.write(sanitized_tex)
        if report["total_bytes_removed"] > 0:
            print(f"[DEFENSE] LaTeX sanitizer: removed {report['total_bytes_removed']} bytes "
                  f"({report['white_text_removed']} white-text blocks, "
                  f"{report['tiny_font_removed']} tiny-font blocks, "
                  f"{report['zero_width_removed']} zero-width blocks)")
        detected = [k for k, v in markers.items() if v]
        if detected:
            print(f"[DEFENSE] Injection markers detected in LaTeX: {detected}")
