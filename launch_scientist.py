...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        help="Attack injection strategy for review robustness testing.",
    )
    parser.add_argument(
        "--attack-target-score",
        type=int,
        default=10,
        choices=range(1, 11),
        help="Target Overall score for attack injection (1-10, default 10).",
    )
    parser.add_argument(
        "--defense",
        action="store_true",
        default=False,
        help="Enable StruQ-structured query defense against prompt injection.",
    )
    parser.add_argument(
        "--defense-level",
        type=str,
        ...
        help="Defense aggressiveness level (default: standard).",
    )
    parser.add_argument(
        "--review-model",
        type=str,
        default=None,
        help="Model to use for paper review. Defaults to the same as --model.",
    )
...
def worker(
    ...
    while True:
        ...
        success = do_idea(
            ...
            log_file=True,
            attack=attack,
            attack_target_score=attack_target_score,
            defense=defense,
...
def do_idea(
    ...
    try:
        ...
        if model == "deepseek-coder-v2-0724":
            ...
            main_model = Model("openai/gpt-pro")
        else:
        ...
        if writeup == "latex":
            ...
            try:
                perform_writeup(idea, folder_name, coder, client, client_model,
                                engine=args.engine, attack=attack,
                                attack_target_score=attack_target_score,
                                defense=defense, defense_level=defense_level)
            except Exception as e:
        ...
        if writeup == "latex":
            try:
                # Use review_model if specified, otherwise fall back to main model
                rev_model = review_model if review_model else client_model
                rev_client, rev_model_name = create_client(rev_model)
                print(f"Review using model: {rev_model_name}")
...
if __name__ == "__main__":
    ...
    if args.attack:
        print(f"[ATTACK MODE] Strategy: {args.attack}, Target Score: {args.attack_target_score}")

    # Create client
    client, client_model = create_client(args.model)

    # Propagate relay settings for Aider/LiteLLM
    if os.environ.get("RELAY_API_KEY") and "OPENAI_API_KEY" not in os.environ:
        os.environ["OPENAI_API_KEY"] = os.environ["RELAY_API_KEY"]
    if os.environ.get("RELAY_BASE_URL") and "OPENAI_API_BASE" not in os.environ:
        os.environ["OPENAI_API_BASE"] = os.environ["RELAY_BASE_URL"]
    ...
    if args.parallel > 0:
        ...
        for i in range(args.parallel):
            ...
            review_model = args.review_model if args.review_model else client_model
        ...
        for idea in novel_ideas:
            ...
            try:
                rev_model = args.review_model if args.review_model else client_model
                success = do_idea(
                    base_dir,
                    results_dir,
                    idea,
